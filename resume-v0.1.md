...
前端：Javascript、HTML、Vue、Typescript
项目管理：禅道
...
- 引入Alchemy进行生产数据库SQL上线和权限管理控制，对于项目信息保密生产数据进行了有效控制【STAR改写一下】
- 搭建了Webhook运维和代码仓合并通知机器人
- 通过pdmanner管理数据库SQL开发


比亚迪公益慈善义卖
项目简介：一共多少人使用，每年举行多少次，每次多少人商品
工作内容：
- 使用Redis Pub\Sub机制解决了多个实例间WebSocket消息无法同步问题，兼容了多实例部署
- 使用RabbitMQ进行业务解耦，搭建了完善的异步、重试、补偿机制，提高了系统的响应速度和稳定性
- 在进行多实例大量日志并发插入时，解决了Mybatis Plus IDWorker雪花算法有一定概率生成主键重复问题，引入Zookeeper进行调控workerid取值，提高了系统并发能力
- 进行了SpringBoot RabbitMQ并发参数调优，断连问题
- 为了更好的记录用户参与互动情况，设计了用户操作埋点方案，便于运营进行用户活跃度和商品库存情况分析
- 使用SkyWalking配合运维进行压测全链路跟踪，进行了系统接口、数据库、网关等层面性能优化，提升了系统并发量
- 使用Nginx配置图片压缩代理，协调企微部门进行流量监控，加快了访问速度，减少了系统网络带宽占用
